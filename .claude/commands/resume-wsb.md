---
description: Rebuild WSB Pulse context from Notion (project status + open tasks) when resuming work
---

# Resume the WSB Pulse project

Rebuild context from **Notion** (the source of truth for status), then orient and ask what to tackle. Do this before proposing any work.

## 1. Pull current status from Notion (authoritative)

- `notion-fetch` the **WallStreetBetsApp** project page: `36bf8289979781bf8347c9eb9e5c04de`. Read its description and the `Tasks` relation.
- Fetch each linked task; for any whose `Status` is **In progress** or **Not started**, note the Task name, Priority, Effort, and body (the bodies hold the real detail and latest dated progress notes).
- Tasks data source: `1cbbd7f0-4917-4a71-91ee-e19f11206d02`. The `notion-task` skill has the IDs/conventions if you need them.

## 2. Quick primer (as of 2026-05-28 — Notion is authoritative; verify against it)

- **Code** `~/Code/n8n/wsb`. Stack: n8n (Reddit scrape → Claude sentiment + daily Buy/Sell/Hold rating) → **Postgres 16** (Docker) → **Next.js 15** dashboard (`web/`, server components query Postgres via `pg`).
...
- **Deploy** is via the `danmwallace.private.wsb` Ansible playbook driven from `~/Code/ansible/ansible-homelab-cfg` (not ad-hoc rsync — rsync clobbers the vaulted `.env`). Live site `wsb.ai.wallace.boston`; n8n at `agents.ai.wallace.boston`.
...
## 3. Likely open threads (confirm against the Notion tasks)
